(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))a(r);new MutationObserver(r=>{for(const o of r)if(o.type==="childList")for(const i of o.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&a(i)}).observe(document,{childList:!0,subtree:!0});function n(r){const o={};return r.integrity&&(o.integrity=r.integrity),r.referrerPolicy&&(o.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?o.credentials="include":r.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(r){if(r.ep)return;r.ep=!0;const o=n(r);fetch(r.href,o)}})();const D="ed2682fe1cd0ecc2efbcdeb7464c5ffd",l={POPULAR_MOVIES:"/movie/popular",UPCOMING_MOVIES:"/movie/upcoming",TRENDING_WEEK:"/trending/movie/week",SEARCH_MOVIES:"/search/movie"};async function O(e,t=1,n="",a=""){return await(await fetch(`https://api.themoviedb.org/3${e}?api_key=${D}&page=${t}&query=${n}&year=${a}`)).json()}async function Z(e){const a=(await(await fetch(`https://api.themoviedb.org/3/movie/${e}/videos?api_key=ed2682fe1cd0ecc2efbcdeb7464c5ffd`)).json()).results.find(r=>r.site==="YouTube"&&r.type==="Trailer");return a?a.key:null}async function C(){const e=`https://api.themoviedb.org/3/genre/movie/list?api_key=${D}&language=en-US`,n=await(await fetch(e)).json(),a={};return n.genres.forEach(r=>{a[r.id]=r.name}),a}const M=document.querySelector(".library-movies-wrapper"),W=document.querySelector("#empty-library"),m=document.querySelector("#loadMore"),N="myLibrary";let w=20,p;function j(e,t){const n=e.map(a=>Q({id:a.id,title:a.title,poster_path:a.poster_path,overview:a.overview,year:a.year,vote_average:a.vote_average,genre_names:a.genres})).join("");t.innerHTML=n,t.onclick=a=>{const r=a.target.closest(".weekly-trends-card");if(!r)return;const o=Number(r.dataset.id),i=e.find(s=>s.id===o);i&&P({id:i.id,title:i.title,poster_path:i.poster_path,vote_average:i.vote_average,vote_count:i.vote_count,popularity:i.popularity,year:i.year,overview:i.overview,genres:i.genres})}}function b(){return JSON.parse(localStorage.getItem(N))||[]}function F(e){const t=b();t.some(n=>n.id===e.id)||(t.push(e),localStorage.setItem(N,JSON.stringify(t)))}function Y(e){const t=b().filter(n=>n.id!==e);localStorage.setItem(N,JSON.stringify(t))}function I(e){return b().some(t=>t.id===e)}async function ee(){const e=b(),t=[...new Set(e.flatMap(a=>a.genres))];p.innerHTML="";const n=document.createElement("option");n.value="",n.textContent="All Genres",p.appendChild(n),t.forEach(a=>{const r=document.createElement("option");r.value=a,r.textContent=a,p.appendChild(r)})}async function U(){const e=b();m.style.display=e.length>20?"block":"none";const t=e.slice(0,w);j(t,M),w>=e.length&&(m.style.display="none")}function te(){b().length===0?(W.style.display="flex",m.style.display="none",p.style.display="none",M.innerHTML=""):(W.style.display="none",p.style.display="block")}function K(e){if(e.length===0){M.innerHTML="<p>No movies found for this genre.</p>",m.style.display="none";return}const t=e.slice(0,w);j(t,M),m.style.display=e.length>w?"block":"none",m.onclick=()=>{w+=20,K(e)}}function ne(){const e=p.value,t=b();w=20;let n=t;e!==""&&(n=t.filter(a=>{const r=a.genres||a.genre_names||[];return(Array.isArray(r)?r:[r]).includes(e)})),K(n)}document.addEventListener("DOMContentLoaded",()=>{p=document.getElementById("genre-filter"),console.log(p),p&&(p.addEventListener("change",ne),ee(),U(),te(),m.addEventListener("click",()=>{w+=20,U()}))});function P(e){const t=document.createElement("div");t.classList.add("popup-backdrop");const n=I(e.id);t.innerHTML=`
    <div class="popup" >
  ...
  `,document.body.appendChild(t),t.querySelector(".popup-close").addEventListener("click",()=>{t.remove()});const a=t.querySelector("#library-toggle-btn");a.addEventListener("click",()=>{I(e.id)?(Y(e.id),a.textContent="Add to my library"):(F(e),a.textContent="Remove from my library")})}function re(){const e=window.innerWidth;return e>=1280?"w500":(e>=780||e>=500,"w342")}const ae=document.getElementById("open-menu"),oe=document.getElementById("close-menu"),J=document.getElementById("mobile-menu");ae.addEventListener("click",()=>{J.classList.add("active")});oe.addEventListener("click",()=>{J.classList.remove("active")});const f="/cinemania/assets/sprite-Phngsp8V.svg";let c=1,h=1,g="",y="",T,B,v,H,k,x,V;function ie(){T=document.querySelector(".catalog-hero-wrapper"),B=document.querySelector(".catalog-filter-input"),v=document.querySelector(".catalog-filter-select"),H=document.querySelector(".catalog-filter-btn"),k=document.querySelector(".catalog-movies-wrapper"),x=document.querySelector("#empty-catalog"),V=document.querySelector("#pagination")}function se(e){return`
  ...
  `}function pe(e,t,n){if(!t)return;const a=e.map(r=>Q({id:r.id,title:r.title,poster_path:r.poster_path,overview:r.overview,year:r.release_date?.split("-")[0]||"—",vote_average:r.vote_average,genres:r.genre_ids.map(o=>n[o]).join(", ")})).join("");t.innerHTML=a}async function E(e,t=1,n="",a=""){if(c=t,g=n.trim(),y=a,!k)return;e===l.SEARCH_MOVIES&&g===""&&a!==""&&(e=l.POPULAR_MOVIES);const r=await O(e,t,n,a),o=await C();h=Math.min(r.total_pages,500);const i=r.results.slice(0,20);i.length===0?(x.style.display="flex",V.style.display="none"):(x.style.display="none",V.style.display="flex"),pe(i,k,o),ue(),k.onclick=s=>{const _=s.target.closest(".weekly-trends-card");if(!_)return;const X=Number(_.dataset.id),d=i.find(A=>A.id===X);d&&P({id:d.id,title:d.title,year:d.release_date.split("-")[0],poster_path:d.poster_path,vote_average:d.vote_average,vote_count:d.vote_count,popularity:d.popularity,overview:d.overview,genres:d.genre_ids.map(A=>o[A])})}}function ue(){const e=document.querySelector(".catalog-pagination");if(!e)return;let t="";t+=`<button class="pg-prev" ${c===1?"disabled":""}>
  ...
  `}async function $e(){if(!G)return;const e=await O(l.UPCOMING_MOVIES),t=await C(),n=e.results[0];n.genre_ids.map(o=>t[o]);const a=_e({id:n.id,title:n.title,backdrop_path:n.backdrop_path,release_date:n.release_date,vote_average:n.vote_average,vote_count:n.vote_count,popularity:n.popularity,overview:n.overview},I(n.id));G.innerHTML=a;const r=document.querySelector("#upcoming-btn");r.addEventListener("click",()=>{I(n.id)?(Y(n.id),r.textContent="Add to my library"):(F(n),r.textContent="Remove from my library")})}document.addEventListener("DOMContentLoaded",()=>{$e(),we()});function Se(){const e=window.innerWidth;return e>=1280?"w500":(e>=780||e>=500,"w342")}function Ee(){const e=window.innerWidth;return e>=1280?"w1280":e>=780?"w780":(e>=500,"w342")}function S(){const e=window.innerWidth;return e>=1280?"18":e>=780?"10":(e>=500,"14")}function Le(){return window.innerWidth>321?3:1}const L=document.querySelector("#theme-toggle");let u=localStorage.getItem("theme")!=="light";document.body.classList.toggle("dark",u);L.classList.toggle("active",u);L.classList.toggle("inactive",!u);L.addEventListener("click",()=>{u=!u,document.body.classList.toggle("dark",u),L.classList.toggle("active",u),L.classList.toggle("inactive",!u),localStorage.setItem("theme",u?"dark":"light")});const ke=window.location.pathname.split("/").pop();document.querySelectorAll(".nav-link").forEach(e=>{e.getAttribute("href").split("/").pop()===ke&&e.classList.add("active")});
//# sourceMappingURL=main-BrDzykYw.js.map
